...
const buildfiles = glob.sync("./src/example6/**/*.ts")
...
module.exports = {
    ...
    output: {
        path: path.resolve(process.cwd(), 'src/example6/dist'),
        filename: '[name].js'
    },
    resolve: {
        extensions: ['.ts','.js'],  
    ...
    plugins:[
        ...
        new TsConfigPathsPlugin({configFileName: "./tsconfig.example6.json",compiler: "typescript"}),
        ...
        new HtmlWebpackPlugin({
            template: path.resolve( process.cwd(), 'src/example6/index.html' ),
            filename: 'index.html',            
            chunksSortMode: 'dependency',
            inject: true
        }),
        new HtmlWebpackPlugin({
            template: path.resolve( process.cwd(), 'src/example6/page1/page1.html' ),
            filename: 'page1.html',
            chunksSortMode: 'dependency',
            inject: true
        }),
        new HtmlWebpackPlugin({
            template: path.resolve( process.cwd(), 'src/example6/page2/page2.html' ),
            filename: 'page2.html',
            chunksSortMode: 'dependency',
            inject: true
        }),
    ...
    devServer: {
        contentBase: path.join(process.cwd(), "src/example6"),
        openPage: './src/example6/index.html',
        port: 3006
    }
}