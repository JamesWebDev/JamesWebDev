...
const typescriptFiles = glob.sync("./src/example8/**/*.ts")
...
module.exports = {
    ...
    output: {
        path: path.resolve(process.cwd(), 'src/example8/dist'),
        filename: '[name].js'
    },
    resolve: {
        extensions: ['.ts','.js'],  
                    ...
                    use: [
                        {loader: 'css-loader'},
                        {
                            ...
                            options: {
                                includePaths: [
                                    path.resolve(process.cwd(), './node_modules'), 
                                    path.resolve(process.cwd(), './src/example8')
                                ]
                            }
                        }
                    ]
    ...
    plugins:[        
        ...
        new HtmlWebpackPlugin({
            template: path.resolve( process.cwd(), 'src/example8/index.html' ),
            filename: 'index.html',            
            chunksSortMode: 'dependency',
            inject: true
        }),
        new HtmlWebpackPlugin({
            template: path.resolve( process.cwd(), 'src/example8/page1/page1.html' ),
            filename: 'page1.html',
            chunksSortMode: 'dependency',
            inject: true
        }),
        new HtmlWebpackPlugin({
            template: path.resolve( process.cwd(), 'src/example8/page2/page2.html' ),
            filename: 'page2.html',
            chunksSortMode: 'dependency',
            inject: true
        }),
    ...
    devServer: {
        contentBase: path.join(process.cwd(), "src/example8"),
        openPage: './src/example8/index.html',
        port: 3008
    }
}